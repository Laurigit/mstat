...
UID_UUSI_PELI_ALL_ROWS <- function(
 ...
 # browser
  Pakka <- UID_PAKKA[Pakka_ID == 3 | Pakka_ID == 36]
  PakkaVS_subset <- UID_PAKKA_VS[Pakka_ID == 3| Pakka_ID == 36]
 ...
 # Aloittaja <- pelidata[Peli_ID == Peli_ID_input , .(Pakka_ID, Aloittaja)]
  ...
  sscols_pakat <- STAT_CURRENT_PAKKA[, .(Pakka_NM, Pakka_ID, Colors, Most_same_card, Most_wins_sames_card,
                                         Pakka_NM_Dynamic, Omistaja_ID)][Pakka_ID == 3 | Pakka_ID == 36]
  ...
  #                           input_right_mulligan,
  #                           STAT_VOITTOENNUSTE)
  ennuste <- STAT_VOITTOENNUSTE[, .(Prediction = ennuste, Peli_ID, Prediction_mirror = 1 - ennuste, Pakka_ID)][Pakka_ID == 3]
  ...
  curr_turn_pelit <- pelidata[Turnaus_NO == max_turnaus , .(Peli_ID,
                                                            ...
                                                            Ottelu_ID,
                                                            Voittaja,
                                                            Aloitus_DT)][Pakka_ID == 3 | Pakka_ID == 36]
