...
text <- "This is a sentence. These are some words after a dot."

# Use str_replace_all to replace all matches with an empty string
result <- str_replace_all(text, "^.*?\\.(?=\\s|$)", "")

# Print the result
print(result)


input_string <- "This is a Test string with Some capitalized Words."
pattern <- "\\b[A-Z][a-z]*\\b"

matches <- grep(pattern, input_string, value = TRUE)
